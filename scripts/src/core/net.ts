...
class HttpRequestObject extends HttpObject implements HttpRequest {
	...
	constructor(properties: RequestProperties) {
		...
		this.url = types.is(properties.url, Url) ? <Url> properties.url : new Url(<string> properties.url);
		...
		/*if (types.is(properties.cors, Boolean) && properties.cors) {
		 this.xhr.withCredentials = true;
		 }*/

		this.future = new types.Future<HttpResponse>();
	}
	...
	protected processDataAndSend(data?: string | collections.FugaziMap<any>): void {
		//const body = data ? JSON.stringify(data instanceof collections.FugaziMap ? data.asObject() : data) : undefined;
		this.xhr.send(/*body*/);
	}
...
class HttpResponseObject extends HttpObject implements HttpResponse {
	...
	public getDataAsJson<T>(): any | T {
		try {
			...
			throw new types.Exception("unable to parse response data");
		}
	}
